...
class StudentMap extends BaseMap
{
    ...
    private function insert(Student $student){
        ...
        if ($this->db->exec("INSERT INTO student(user_id, gruppa_id, num_zach) VALUES($student->user_id, $student->gruppa_id, $numZach)") == 1) {
            return true;
        }
        return false;
    }
    ...
    private function update(Student $student){
        $numZach = $this-> db->quote($student->num_zach);
        if ($this->db->exec("UPDATE student SET gruppa_id = $student->gruppa_id, num_zach= $numZach WHERE user_id=".$student->user_id) == 1) {
            return true;
        }
        return false;
    }
